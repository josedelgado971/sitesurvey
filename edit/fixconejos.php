<?php
    ...
    if($_POST['ident'] != null && $_POST['alojamiento'] != null && $_POST['alimento'] != null && $_POST['cuidados'] != null && $_POST['otros'] != null){
        ...
        $sql = "UPDATE conejos SET ident = '" . trim($_POST['ident']) . "' ,alojamiento = '" . trim($_POST['alojamiento']) . "' ,alimento = '" . trim($_POST['alimento']) . "' ,cuidados = '" . trim($_POST['cuidados']) . "' ,otros = '" . trim($_POST['otros']) . "' WHERE id = '" . $_POST['id'] . "'";
        if($conn->query($sql)){
            echo "<script>alert('Proceso completado exitósamente')</script>";
            header("Refresh:0 , url =../categorias/conejos.php");
            exit();
