<?php
    ...
    if($_POST['ident'] != null && $_POST['alojamiento'] != null && $_POST['alimento'] != null && $_POST['cuidados'] != null && $_POST['otros'] != null ){
        ...
        $sql = "INSERT INTO conejos (ident,alojamiento,alimento,cuidados,otros) VALUES ('" .trim($_POST['ident']). "' ,'" .trim($_POST['alojamiento']). "' ,'" .trim($_POST['alimento']). "' ,'" .trim($_POST['cuidados']). "' ,'" .trim($_POST['otros']). "')";
        if($conn->query($sql)){
            echo "<script>alert('agregado exitosamente')</script>";
            header("Refresh:0 , url = ../categorias/userconejos.php");
            exit();
